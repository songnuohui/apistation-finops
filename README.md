...
当前目录已包含可运行的 MVP 原型。技术路线为 Node.js 22 单服务：同一进程承载 HTTP API、周期同步器和静态管理前端，复用现有 PostgreSQL 实例并使用独立 `finops` Schema。FinOps 不参与转发和扣费主链路，只读访问 `sub2api` 所需源字段，不读取账号凭证，也不修改用户余额。请求级用量不再同步或落库到 FinOps；页面通过 Sub2API 管理员聚合接口按需只读查询，并使用请求合并、短时缓存、过期缓存回退和最多 2 个并发保护源服务。
...
- FinOps 对 `public` 源 Schema 只有用户、账号和资金等所需列的 `SELECT`，不再拥有 `usage_logs` 读取权限，只在自己的 `finops` Schema 写入业务台账。
...
- 已实现 `002_cny_accounting`、只读预检、用户/账号/支付订单增量同步、Sub2API 用量聚合 API 按需查询、演示模式、CSV 导出、供应商采购聚合、来源级同步状态及成本档案录入。
- 已实现独立 `/monitor` 分组可用性监控页：管理端可配置展示的 `source_group_id`、名称、模型标签、展示倍率、探测参数和独立刷新间隔；公开页可直接打开，也可在 FinOps 或上游控制台页面中通过 iframe 嵌入。FinOps 只读同步 Sub2API 分组目录与倍率，独立执行分组探测，并在自有数据库中保留每个监控/模型最近 60 次明细与最近 30 天日聚合；不会写入或修改 Sub2API 数据库、Redis、服务或监控配置。
