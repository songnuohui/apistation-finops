...
export class DemoRepository {
  ...
  async getEmailCampaign(id) { const campaign = this.emailCampaigns.find((item) => Number(item.id) === Number(id)); if (!campaign) throw Object.assign(new Error('email campaign not found'), { statusCode: 404 }); const recipients = (this.emailRecipients.get(Number(id)) || []).map((item) => ({ ...item, errorMessage: item.errorMessage || '' })); return { ...campaign, reviewCount: recipients.filter((item) => item.status === 'needs_review').length, recipients }; }
  ...
  async recoverInterruptedEmailCampaigns() { let count = 0; for (const campaign of this.emailCampaigns.filter((item) => item.status === 'sending')) { const recipients = this.emailRecipients.get(Number(campaign.id)) || []; recipients.filter((item) => item.status === 'pending').forEach((item) => { item.status = 'needs_review'; item.errorMessage = '发送过程被中断，SMTP 结果待管理员确认'; }); campaign.status = 'interrupted'; await this.finishEmailCampaign(campaign.id); count += 1; } return count; }
