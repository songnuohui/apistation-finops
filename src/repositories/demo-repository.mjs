...
export class DemoRepository {
  ...
  async listEmailPreferences({ page = 1, pageSize = 20, search = '', whitelist = 'all', subscribed = 'all' } = {}) {
    ...
    const items = this.users.filter((user) => `${user.id} ${user.email} ${user.username}`.toLowerCase().includes(term)).map((user) => {
      const preference = this.emailPreferences.get(Number(user.id));
      return { sourceUserId: Number(user.id), email: user.email || '', username: user.username || '', excludeFromBalanceStats: Boolean(user.excludeFromBalanceStats), active: !user.status || user.status === 'active', subscribed: preference?.subscribed !== false, unsubscribedAt: preference?.unsubscribedAt || null, resubscribedAt: preference?.resubscribedAt || null };
  ...
  async getEmailCampaign(id) { const campaign = this.emailCampaigns.find((item) => Number(item.id) === Number(id)); if (!campaign) throw Object.assign(new Error('email campaign not found'), { statusCode: 404 }); const recipients = (this.emailRecipients.get(Number(id)) || []).map((item) => ({ ...item, errorMessage: item.errorMessage || '' })); return { ...campaign, reviewCount: recipients.filter((item) => item.status === 'needs_review').length, pendingCount: recipients.filter((item) => item.status === 'pending').length, recipients }; }
  async markEmailCampaignSending(id) { const campaign = await this.getEmailCampaign(id); if (campaign.status !== 'draft') throw Object.assign(new Error('邮件活动不存在或已发送'), { statusCode: 409 }); campaign.status = 'sending'; campaign.deliveryVersion = 2; Object.assign(this.emailCampaigns.find((item) => item.id === campaign.id), campaign); return campaign; }
  async resumeEmailCampaign(id) { const campaign = await this.getEmailCampaign(id); if (campaign.status !== 'interrupted' || !campaign.pendingCount) throw Object.assign(new Error('没有可以继续发送的未尝试收件人'), { statusCode: 409 }); campaign.status = 'sending'; campaign.deliveryVersion = 2; Object.assign(this.emailCampaigns.find((item) => item.id === campaign.id), campaign); return campaign; }
  ...
  async markEmailRecipientSending(id) { for (const list of this.emailRecipients.values()) { const item = list.find((entry) => String(entry.id) === String(id) && entry.status === 'pending'); if (item) { item.status = 'sending'; item.errorMessage = ''; return true; } } return false; }
  ...
  async recoverInterruptedEmailCampaigns() { let count = 0; for (const campaign of this.emailCampaigns.filter((item) => item.status === 'sending')) { const recipients = this.emailRecipients.get(Number(campaign.id)) || []; recipients.filter((item) => item.status === 'sending' || (campaign.deliveryVersion !== 2 && item.status === 'pending')).forEach((item) => { item.status = 'needs_review'; item.errorMessage = '发送过程被中断，SMTP 结果待管理员确认'; }); campaign.status = 'interrupted'; await this.finishEmailCampaign(campaign.id); count += 1; } return count; }
  async markEmailCampaignInterrupted(id, errorMessage = '') { const campaign = this.emailCampaigns.find((item) => Number(item.id) === Number(id)); if (!campaign) throw Object.assign(new Error('email campaign not found'), { statusCode: 404 }); (this.emailRecipients.get(Number(id)) || []).filter((item) => item.status === 'sending').forEach((item) => { item.status = 'needs_review'; item.errorMessage = errorMessage || '发送过程被中断，SMTP 结果待管理员确认'; }); campaign.status = 'interrupted'; return this.finishEmailCampaign(id); }
  async confirmEmailRecipientSent(campaignId, recipientId, actor = 'admin') { const recipient = (this.emailRecipients.get(Number(campaignId)) || []).find((item) => String(item.id) === String(recipientId) && item.status === 'needs_review'); if (!recipient) throw Object.assign(new Error('收件人不是待确认状态，或邮件活动不存在'), { statusCode: 409 }); recipient.status = 'sent'; recipient.errorMessage = ''; recipient.sentAt = recipient.sentAt || new Date().toISOString(); recipient.reviewedAt = new Date().toISOString(); recipient.reviewedBy = actor; return this.finishEmailCampaign(campaignId); }
  async finishEmailCampaign(id) { const campaign = this.emailCampaigns.find((item) => Number(item.id) === Number(id)); const recipients = this.emailRecipients.get(Number(id)) || []; campaign.sentCount = recipients.filter((item) => item.status === 'sent').length; campaign.failedCount = recipients.filter((item) => item.status === 'failed').length; campaign.skippedCount = recipients.filter((item) => item.status.startsWith('skipped_')).length; campaign.reviewCount = recipients.filter((item) => item.status === 'needs_review').length; campaign.status = campaign.reviewCount ? 'interrupted' : campaign.failedCount ? (campaign.sentCount ? 'partial_failed' : 'failed') : 'completed'; if (!campaign.reviewCount) campaign.sentAt = campaign.sentAt || new Date().toISOString(); return { ...campaign }; }
}
