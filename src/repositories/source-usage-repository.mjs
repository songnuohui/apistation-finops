...
export class SourceUsageRepository {
  ...
  cacheKey({ start, end, accountIds }) {
    ...
    return `${new Date(start).toISOString()}:${endBucket}:${ids.length ? ids.join(',') : '*'}`;
  }
  ...
  async getDailyAccountGroupStats({ start, end, accountIds = null }) {
    ...
    const key = this.cacheKey({ start, end, accountIds: ids });
    const now = Date.now();
    ...
    const load = (async () => {
      ...
      try {
        ...
        result = await client.query(`
          ...
          ORDER BY account_id,day,group_id
        `, args);
      ...
      const value = result.rows.map((row) => ({
        ...
        accountCost: number(row.account_cost),
      }));
