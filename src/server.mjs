...
import {
  ...
  normalizeModelAuditNotificationId, normalizeModelAuditNotificationIds,
  normalizeModelAuditNotificationConfirmAll,
...
async function api(request,res,url){
  ...
  if(request.method==='GET'&&url.pathname==='/api/model-audit/mappings'){
    return json(res,200,await modelAuditRepository.listMappings(page()));
  }
  ...
  if(request.method==='POST'&&url.pathname==='/api/model-audit/test-run'){
    return json(res,202,await modelAuditService.runTest(
  ...
  if(request.method==='GET'&&url.pathname==='/api/model-audit/notifications'){
    const status=url.searchParams.get('status')||'';
    if(status && !['pending','sending','needs_confirmation','sent','failed','skipped'].includes(status)){
  ...
  const modelAuditNotificationConfirmId=/^\/api\/model-audit\/notifications\/(\d+)\/confirm$/.exec(url.pathname);
  if(request.method==='POST'&&modelAuditNotificationConfirmId){
    return json(res,200,await modelAuditRepository.confirmNotification(
      normalizeModelAuditNotificationId(modelAuditNotificationConfirmId[1]),
      auth.actor,
    ));
  }
  if(request.method==='POST'&&url.pathname==='/api/model-audit/notifications/confirm'){
    return json(res,200,await modelAuditRepository.confirmNotifications(
      normalizeModelAuditNotificationIds(await body(request)).ids,
      auth.actor,
    ));
  }
  if(request.method==='POST'&&url.pathname==='/api/model-audit/notifications/confirm-all'){
    return json(res,200,await modelAuditRepository.confirmAllNotifications(
      normalizeModelAuditNotificationConfirmAll(await body(request)),
      auth.actor,
    ));
  }
  ...
  const emailRecipientConfirm=/^\/api\/email\/campaigns\/(\d+)\/recipients\/(\d+)\/confirm$/.exec(url.pathname);
  if(request.method==='POST'&&emailRecipientConfirm){
    return json(res,200,await repository.confirmEmailRecipientSent(Number(emailRecipientConfirm[1]),Number(emailRecipientConfirm[2]),auth.actor));
  }
  const emailCampaignResume=/^\/api\/email\/campaigns\/(\d+)\/resume$/.exec(url.pathname);
  if(request.method==='POST'&&emailCampaignResume) return json(res,202,await emailService.queueCampaign(Number(emailCampaignResume[1]),{ resume:true }));
  const emailCampaignSend=/^\/api\/email\/campaigns\/(\d+)\/send$/.exec(url.pathname);
  if(request.method==='POST'&&emailCampaignSend) return json(res,202,await emailService.queueCampaign(Number(emailCampaignSend[1])));
  ...
  if(request.method==='DELETE'&&periodId){
    return json(res,200,await repository.deleteAccountCostPeriod(
      periodId,normalizeAccountCostPeriodDelete(await body(request)),auth.actor,
    ));
  }
...
async function readiness(){
  ...
  const cleanupMigration=await finopsPool.query(
    `SELECT 1 FROM "${config.finopsSchema}".schema_migrations WHERE version=$1`,
    ['059_remove_qq_alerts'],
  );
  if(!cleanupMigration.rowCount)throw new Error('required FinOps migration 059_remove_qq_alerts is not applied');
  const voidCostPeriodMigration=await finopsPool.query(
    `SELECT 1 FROM "${config.finopsSchema}".schema_migrations WHERE version=$1`,
  ...
  const emailRecoveryMigration=await finopsPool.query(
    `SELECT 1 FROM "${config.finopsSchema}".schema_migrations WHERE version=$1`,
    ['063_finops_email_interruption_recovery'],
  );
  if(!emailRecoveryMigration.rowCount)throw new Error('required FinOps migration 063_finops_email_interruption_recovery is not applied');
  const emailBackgroundMigration=await finopsPool.query(
    `SELECT 1 FROM "${config.finopsSchema}".schema_migrations WHERE version=$1`,
    ['064_finops_email_background_delivery'],
  );
  if(!emailBackgroundMigration.rowCount)throw new Error('required FinOps migration 064_finops_email_background_delivery is not applied');
  const monitorDisplayMultiplierMigration=await finopsPool.query(
    `SELECT 1 FROM "${config.finopsSchema}".schema_migrations WHERE version=$1`,
  ...
  const monitorGroupRefreshConfigMigration=await finopsPool.query(
    `SELECT 1 FROM "${config.finopsSchema}".schema_migrations WHERE version=$1`,
  ...
  const modelAuditTestRunsMigration=await finopsPool.query(
    `SELECT 1 FROM "${config.finopsSchema}".schema_migrations WHERE version=$1`,
    ['070_model_audit_test_runs'],
  );
  if(!modelAuditTestRunsMigration.rowCount)throw new Error('required FinOps migration 070_model_audit_test_runs is not applied');
  const modelAuditUserNotificationsMigration=await finopsPool.query(
    `SELECT 1 FROM "${config.finopsSchema}".schema_migrations WHERE version=$1`,
    ['071_model_audit_user_notifications'],
  );
  if(!modelAuditUserNotificationsMigration.rowCount)throw new Error('required FinOps migration 071_model_audit_user_notifications is not applied');
  const modelAuditNotificationConfirmationMigration=await finopsPool.query(
    `SELECT 1 FROM "${config.finopsSchema}".schema_migrations WHERE version=$1`,
    ['072_model_audit_notification_confirmation'],
  );
  if(!modelAuditNotificationConfirmationMigration.rowCount)throw new Error('required FinOps migration 072_model_audit_notification_confirmation is not applied');
  const monitorAnnouncementMigration=await finopsPool.query(
    `SELECT 1 FROM "${config.finopsSchema}".schema_migrations WHERE version=$1`,
