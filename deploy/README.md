...
监控配置、展示倍率、刷新设置和 FinOps 自己的探测结果写入独立 FinOps 数据库；Sub2API 仅通过只读连接提供分组目录及当前倍率。FinOps 为每个监控/模型保留最近 60 次明细和最近 30 天日聚合，并支持 7/15/30 天统计。管理员可以在每个分组的编辑窗口独立保存历史调整参数；只有点击“应用历史调整”才会在一次事务中修改已有 FinOps 历史状态和日报汇总，并保存前后快照。后续探针仍会继续写入并自然改变百分比；最近一次调整可在没有后续数据冲突时撤销。删除监控会级联删除对应历史，重新配置探测参数会清空该监控历史并从新的配置时间重新开始；不会修改源服务、源数据库或源 Redis。
