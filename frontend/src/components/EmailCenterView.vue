...
async function sendCampaign() {
  ...
  try {
    ...
    closeEditor(); await load(); detail.value = { ...result, recipients: [] }; emit('toast', '邮件已加入后台发送队列，关闭窗口后仍会继续发送');
  } catch (error: any) { emit('toast', error.message); }
...
async function resumeCampaign() {
  if (!detail.value || !window.confirm('继续发送尚未尝试的收件人吗？已发送或待确认的收件人不会重复发送。')) return;
  try { await send(`/email/campaigns/${detail.value.id}/resume`, 'POST', {}); await load(); detail.value = await get(`/email/campaigns/${detail.value.id}`); emit('toast', '未尝试的收件人已加入后台发送队列'); }
  catch (error: any) { emit('toast', error.message); }
}
...
function recipientStatusLabel(value: any) { const labels: Record<string, string> = { pending: '等待后台发送', sending: '正在投递', needs_review: '送达待确认', sent: '已发送', failed: '发送失败', skipped_whitelist: '跳过：白名单', skipped_unsubscribed: '跳过：已退订', skipped_inactive: '跳过：用户已停用', skipped_invalid: '跳过：邮箱无效' }; return labels[String(value)] || String(value || '--'); }
...
.email-resume-row{display:flex;align-items:center;justify-content:space-between;gap:12px;margin:-6px 0 16px;color:var(--muted);font-size:12px}@media(max-width:760px){.email-resume-row{align-items:stretch;flex-direction:column}.email-resume-row button{justify-content:center}}
</style>
