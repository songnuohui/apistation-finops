...
import { Bold, Image, Italic, Link, List, Mail, RefreshCw, Send, Settings2, Underline, X } from 'lucide-vue-next';
...
function toggleAllUsers(checked: boolean) { const next = new Set(selectedUsers.value); preferenceRows.value.forEach((row: AnyRecord) => { if (row.excludeFromBalanceStats || !row.email) return; checked ? next.add(Number(row.sourceUserId)) : next.delete(Number(row.sourceUserId)); }); selectedUsers.value = next; if (editor.value) editor.value.userIds = [...next]; }
...
function recipientStatusLabel(value: any) { const labels: Record<string, string> = { pending: '待发送', sent: '已发送', failed: '发送失败', skipped_whitelist: '跳过：白名单', skipped_unsubscribed: '跳过：已退订', skipped_inactive: '跳过：用户已停用', skipped_invalid: '跳过：邮箱无效' }; return labels[String(value)] || String(value || '--'); }
...
.email-campaign-table{min-width:950px!important}.filter-select{display:flex;align-items:center;gap:6px;height:37px;color:var(--muted);font-size:11px;white-space:nowrap}.filter-select select{height:37px;padding:0 25px 0 9px;border:1px solid var(--line);border-radius:7px;background:#fff;color:var(--ink);font:inherit}.email-pager{gap:10px}.email-pager label{display:flex;align-items:center;gap:5px;margin-right:auto;color:var(--muted);font-size:11px}.email-pager select{height:30px;padding:0 21px 0 7px;border:1px solid var(--line);border-radius:6px;background:#fff;color:var(--ink);font:inherit}.email-detail-meta{margin:-8px 0 16px;color:var(--muted);font-size:11px}@media(max-width:760px){.email-preference-actions .search-box{width:100%;flex:1}.filter-select{flex:1}.filter-select select{width:100%}}
</style>
