...
const formatCny = (value: any) => value === null || value === undefined || value === ''
  ? '--'
...
async function saveAccount() {
  ...
  try {
    const multiplierMode = ['manual_multiplier', 'probe_multiplier'].includes(accountEditor.value.costMode);
    const changeStrategy = multiplierMode ? accountEditor.value.changeStrategy : 'future_only';
    const customEffectiveFrom = changeStrategy === 'custom_time'
      ? new Date(accountEditor.value.customEffectiveFrom)
      : null;
...
function openSupplier() {
  supplierEditor.value = {
    ...
    username: '', password: '', accessToken: '', refreshToken: '', enabled: true, qualityMonitorMode: 'hybrid',
  };
}
  ...
  try {
    await send('/supplier-connections', 'POST', {
      ...
      credentials: {
        ...
        refreshToken: supplierEditor.value.refreshToken,
      },
    });
...
<template>
  <div class="app-shell">
    ...
    <div v-if="accountEditor" class="modal-layer" @click.self="accountEditor = null"><section class="modal form-modal"><header><div><h2>编辑账号成本</h2><p>{{ accountEditor.name }}</p></div><button class="icon-button" @click="accountEditor = null"><X :size="19" /></button></header><div class="form-grid"><label>成本模式<select v-model="accountEditor.costMode"><option value="probe_multiplier">供应商密钥倍率（自动）</option><option value="manual_multiplier">手动填写倍率</option><option value="fixed_purchase">固定采购成本</option><option value="free">免费账号</option></select></label><label v-if="['manual_multiplier','probe_multiplier'].includes(accountEditor.costMode)">变更范围<select v-model="accountEditor.changeStrategy"><option value="future_only">仅未来消费</option><option value="current_day">从今天 0 点开始</option><option value="custom_time">自定义日期和时间</option></select></label><label v-if="['manual_multiplier','probe_multiplier'].includes(accountEditor.costMode) && accountEditor.changeStrategy === 'custom_time'">开始计算时间<input v-model="accountEditor.customEffectiveFrom" type="datetime-local" step="60" /></label><label v-if="accountEditor.costMode === 'manual_multiplier'">进货倍率<input v-model="accountEditor.upstreamMultiplier" type="number" min="0" step="0.0001" placeholder="例如 0.5" /></label><label>成本供应商<input v-model="accountEditor.supplier" placeholder="可选" /></label><label>采购批次 / 密钥<input v-model="accountEditor.purchaseBatch" placeholder="关联供应商密钥或采购批次" /></label><label v-if="accountEditor.costMode === 'fixed_purchase'">说明<input v-model="accountEditor.currentCostNotes" placeholder="固定成本请在成本期间中维护" /></label></div><div class="form-note">供应商密钥关联后直接使用密钥倍率自动计算成本；这里不填写销售倍率。</div><footer><button class="secondary-button" @click="accountEditor = null">取消</button><button class="primary-button" @click="saveAccount">保存成本规则</button></footer></section></div>
    <div v-if="supplierEditor" class="modal-layer" @click.self="supplierEditor = null"><section class="modal form-modal"><header><div><h2>添加供应商连接</h2><p>凭据只会加密保存在 FinOps 自己的数据库</p></div><button class="icon-button" @click="supplierEditor = null"><X :size="19" /></button></header><div class="form-grid"><label>供应商名称<input v-model="supplierEditor.supplierName" placeholder="例如 Sub2API 主站" /></label><label>连接名称<input v-model="supplierEditor.name" placeholder="例如 主账号" /></label><label>类型<select v-model="supplierEditor.adapterType"><option value="sub2api">Sub2API</option><option value="newapi">NewAPI</option><option value="auto">自动识别</option></select></label><label>上游地址<input v-model="supplierEditor.baseUrl" placeholder="https://..." /></label><label>认证方式<select v-model="supplierEditor.authMode"><option value="password">账号密码</option><option v-if="supplierEditor.adapterType === 'sub2api'" value="token_refresh">Access Token + Refresh Token</option><option value="access_token">静态访问令牌</option></select></label><label v-if="supplierEditor.authMode === 'password'">管理员账号<input v-model="supplierEditor.username" autocomplete="off" /></label><label v-if="supplierEditor.authMode === 'password'">管理员密码<input v-model="supplierEditor.password" type="password" autocomplete="new-password" /></label><template v-else-if="supplierEditor.authMode === 'token_refresh'"><label>Access Token<input v-model="supplierEditor.accessToken" type="password" autocomplete="off" /></label><label>Refresh Token<input v-model="supplierEditor.refreshToken" type="password" autocomplete="off" /></label></template><label v-else>静态访问令牌<input v-model="supplierEditor.accessToken" type="password" autocomplete="new-password" /></label><label>评分模式<select v-model="supplierEditor.qualityMonitorMode"><option value="hybrid">被动 + 主动探测</option><option value="passive">只使用真实消费</option><option value="active">只使用主动探测</option></select></label><label>供应商备注<textarea v-model="supplierEditor.supplierNotes" rows="3" maxlength="2000" placeholder="采购联系人、价格说明或其他内部备注"></textarea></label></div><div class="form-note">FinOps 只读取供应商接口和 sub2api Redis 的实时数据，不会修改上游系统。</div><footer><button class="secondary-button" @click="supplierEditor = null">取消</button><button class="primary-button" @click="saveSupplier">创建连接</button></footer></section></div>
  </div>
</template>
